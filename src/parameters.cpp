...
std::string laser_map_topic = "laser_map";
std::string aft_mapped_to_init_topic = "aft_mapped_to_init";
std::string path_topic = "path";
// END

// Frame name
std::string camera_init_frame = "camera_init";
std::string body_frame = "body";
std::string aft_mapped_frame = "aft_mapped";
// END
...
double                 time_update_last = 0.0, time_current = 0.0,
       time_predict_last_const = 0.0, t_last = 0.0;
...
void readParameters(std::shared_ptr<rclcpp::Node>& nh)
{
  ...
  try
  {
    ...
    // Topic name
    nh->declare_parameter<std::string>("common.cloud_registered_topic",
                                       "cloud_registered");
    nh->get_parameter("common.cloud_registered_topic", cloud_registered_topic);
    ...
    nh->declare_parameter<std::string>("common.cloud_registered_body_topic",
                                       "cloud_registered_body");
    nh->get_parameter("common.cloud_registered_body_topic",
                      cloud_registered_body_topic);

    nh->declare_parameter<std::string>("common.laser_map_topic", "laser_map");
    nh->get_parameter("common.laser_map_topic", laser_map_topic);

    nh->declare_parameter<std::string>("common.aft_mapped_to_init_topic",
                                       "aft_mapped_to_init");
    nh->get_parameter("common.aft_mapped_to_init_topic",
                      aft_mapped_to_init_topic);

    nh->declare_parameter<std::string>("common.path_topic", "path");
    nh->get_parameter("common.path_topic", path_topic);

    // END

    // Frame name

    nh->declare_parameter<std::string>("common.camera_init_frame",
                                       "camera_init");
    nh->get_parameter("common.camera_init_frame", camera_init_frame);

    nh->declare_parameter<std::string>("common.body_frame", "body");
    nh->get_parameter("common.body_frame", body_frame);

    nh->declare_parameter<std::string>("common.aft_mapped_frame", "aft_mapped");
    nh->get_parameter("common.aft_mapped_frame", aft_mapped_frame);
    ...
    nh->get_parameter("common.cut_frame_time_interval",
    ...
    nh->declare_parameter<std::vector<double>>("prior_pcd.init_pose",
                                               std::vector<double>());
    ...
    nh->declare_parameter<bool>("odometry.publish_odometry_without_downsample",
                                false);
    nh->get_parameter("odometry.publish_odometry_without_downsample",
    ...
    nh->get_parameter("mapping.ivox_grid_resolution",
