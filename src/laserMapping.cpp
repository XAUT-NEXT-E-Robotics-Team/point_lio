...
V3D euler_cur; // Current euler angles for logging purposes
...
PointCloudXYZI::Ptr loadPointcloudFromPcd(const std::string& file_path)
{
  ...
  RCLCPP_INFO(
      LOGGER, "Loaded %zu points from %s", pcd_ptr->size(), file_path.c_str());
  return pcd_ptr;
}
...
void pointBodyLidarToIMU(PointType const* const pi, PointType* const po)
{
  ...
  if (extrinsic_est_en)
  {
    ...
    {
      p_body_imu =
          kf_output.x_.offset_R_L_I * p_body_lidar + kf_output.x_.offset_T_L_I;
    }
    else
    {
      p_body_imu =
          kf_input.x_.offset_R_L_I * p_body_lidar + kf_input.x_.offset_T_L_I;
    }
  }
  else
  {
        ...
        if (imu_en)
        {
...

          {
            while (
                ...
                get_time_sec(
                    imu_next.header.stamp)) // if it is needed for the new map?
            {
              imu_deque.pop_front();
              if (imu_deque.empty())
              {
        ...
        if (!time_seq.empty())
        {
          ...
          for (k = 0; k < time_seq.size(); k++)
          {
                ...
                get_time_sec(imu_next.header.stamp)) // && !imu_deque.empty())
            {
              ...
              double dt_cov =
                  get_time_sec(imu_last.header.stamp) - time_update_last;
              if (dt_cov > 0.0)
              {
            ...
            { // >= ?
              ...
              if (!is_first_frame)
              {
                ...
                if (dt_cov > 0.0)
                {
                  // kf_input.predict(dt_cov, Q_input, input_in, false, true);
                  time_update_last =
                      get_time_sec(imu_next.header.stamp); // time_current;
                }
                // kf_input.predict(dt, Q_input, input_in, true, false);
